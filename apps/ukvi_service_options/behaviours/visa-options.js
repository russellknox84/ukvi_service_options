...
const visaTypes = require('./visa-types');
...
const checkCondition = (conditions, session) => {
    if (!conditions) {
        return true;
    }
    const conditionToCalculate = conditions.split(' OR ');
    const conditionToMeet = conditionToCalculate.map(condition => {
